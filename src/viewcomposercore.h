...
/**
 * @brief The ComposerViewCore class
 * It is inspired by ComposerViewBase, hopefull at some point can be used as base class for
 * ComposerViewBase.
 */
class ComposerViewCore : public QObject
{
    Q_OBJECT
    
    /// Attachment model
    Q_PROPERTY(AttachmentModel *attachmentModel READ attachmentModel WRITE setAttachmentModel NOTIFY attachmentModelChanged)
    
    /// The selected identity from whom the message will be sent.
    Q_PROPERTY(int identityUoid READ identityUoid WRITE setIdentityUoid NOTIFY identityUoidChanged)
    
    Q_PROPERTY(QString text READ text WRITE setText NOTIFY textChanged)
    
public:
    explicit ComposerViewCore(QObject *parent = nullptr);
    ~ComposerViewCore() override;
