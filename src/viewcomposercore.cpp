/*
  ...
  SPDX-FileCopyrightText: 2021 Carl Schwan <carlschwan.kdeorg>

  SPDX-License-Identifier: LGPL-2.0-or-later
*/

#include "viewcomposercore.h"
...
#include <MessageComposer/ImageScalingUtils>
#include <MessageComposer/EmailAddressResolveJob>
...
ComposerViewCore::ComposerViewCore(QObject *parent)
    : QObject(parent)
...
ComposerViewCore::~ComposerViewCore() = default;

bool ComposerViewCore::isComposing() const
{
    return !m_composers.isEmpty();
}

void ComposerViewCore::setMessage(const KMime::Message::Ptr &msg, bool allowDecryption)
{
...
void ComposerViewCore::updateTemplate(const KMime::Message::Ptr &msg)
{
    // First, we copy the message and then parse it to the object tree parser.
    // The otp gets the message text out of it, in textualContent(), and also decrypts
    // the message if necessary.
...
void ComposerViewCore::saveMailSettings()
{
    const KIdentityManagement::Identity identity = identityManager()->identityForUoid(identityUoid());
...
void ComposerViewCore::clearFollowUp()
{
    mFollowUpDate = QDate();
    mFollowUpCollection = Akonadi::Collection();
}

void ComposerViewCore::send(MessageComposer::MessageSender::SendMethod method, MessageComposer::MessageSender::SaveIn saveIn, bool checkMailDispatcher)
{
    mSendMethod = method;
    mSaveIn = saveIn;
...
void ComposerViewCore::setCustomHeader(const QMap<QByteArray, QString> &customHeader)
{
    m_customHeader = customHeader;
}

void ComposerViewCore::readyForSending()
{
    ...
    connect(job, &MessageComposer::EmailAddressResolveJob::result, this, &ComposerViewCore::slotEmailAddressResolved);
    job->start();
}

void ComposerViewCore::slotEmailAddressResolved(KJob *job)
{
    if (job->error()) {
    ...
    foreach (MessageComposer::Composer *composer, m_composers) {
        ...
        connect(composer, &MessageComposer::Composer::result, this, &ComposerViewCore::slotSendComposeResult);
        composer->start();
...
bool ComposerViewCore::addKeysToContext(const QString &gnupgHome,
...
QVector<MessageComposer::Composer *> ComposerViewCore::generateCryptoMessages(bool &wasCanceled)
{
    ...
    if (composers.isEmpty() && (signSomething || encryptSomething)) {
        Q_ASSERT_X(false, "ComposerViewCore::fillCryptoInfo", "No concrete sign or encrypt method selected");
    }

    return composers;
}

void ComposerViewCore::fillGlobalPart(MessageComposer::GlobalPart *globalPart)
{
...
void ComposerViewCore::fillInfoPart(MessageComposer::InfoPart *infoPart, ComposerViewCore::RecipientExpansion expansion)
{
...
void ComposerViewCore::slotSendComposeResult(KJob *job)
{
...
void ComposerViewCore::saveRecentAddresses(const KMime::Message::Ptr &msg)
{
...
void ComposerViewCore::queueMessage(const KMime::Message::Ptr &message, MessageComposer::Composer *composer)
{
    ...
    message->assemble();
    connect(qjob, &MailTransport::MessageQueueJob::result, this, &ComposerViewCore::slotQueueResult);
    m_pendingQueueJobs++;
    qjob->start();
...
void ComposerViewCore::slotQueueResult(KJob *job)
{
    m_pendingQueueJobs--;
...
void ComposerViewCore::fillQueueJobHeaders(MailTransport::MessageQueueJob *qjob, KMime::Message::Ptr message, const MessageComposer::InfoPart *infoPart)
{
...
void ComposerViewCore::initAutoSave()
{
...
Akonadi::Collection ComposerViewCore::followUpCollection() const
{
    return mFollowUpCollection;
}

void ComposerViewCore::setFollowUpCollection(const Akonadi::Collection &followUpCollection)
{
    mFollowUpCollection = followUpCollection;
}

QDate ComposerViewCore::followUpDate() const
{
    return mFollowUpDate;
}

void ComposerViewCore::setFollowUpDate(const QDate &followUpDate)
{
    mFollowUpDate = followUpDate;
}

Sonnet::DictionaryComboBox *ComposerViewCore::dictionary() const
{
    return m_dictionary;
}

void ComposerViewCore::setDictionary(Sonnet::DictionaryComboBox *dictionary)
{
    m_dictionary = dictionary;
}

void ComposerViewCore::updateAutoSave()
{
    if (m_autoSaveInterval == 0) {
        ...
        if (!m_autoSaveTimer) {
            ...
            if (m_parentWidget) {
                ...
                connect(m_autoSaveTimer, &QTimer::timeout, this, &ComposerViewCore::autoSaveMessage);
            }
        }
...
void ComposerViewCore::cleanupAutoSave()
{
    delete m_autoSaveTimer;
    m_autoSaveTimer = nullptr;
    if (!m_autoSaveUUID.isEmpty()) {
...
//-----------------------------------------------------------------------------
void ComposerViewCore::autoSaveMessage()
{
    ...
    composer->setAutocryptEnabled(id.autocryptEnabled());
    m_composers.append(composer);
    connect(composer, &MessageComposer::Composer::result, this, &ComposerViewCore::slotAutoSaveComposeResult);
    composer->start();
}

void ComposerViewCore::setAutoSaveFileName(const QString &fileName)
{
...
void ComposerViewCore::slotAutoSaveComposeResult(KJob *job)
{
...
void ComposerViewCore::writeAutoSaveToDisk(const KMime::Message::Ptr &message)
{
...
void ComposerViewCore::addCustomHeaders(const KMime::Message::Ptr &message)
{
...
void ComposerViewCore::saveMessage(const KMime::Message::Ptr &message, MessageComposer::MessageSender::SaveIn saveIn)
{
    ...
    if (!identity.isNull()) { // we have a valid identity
        ...
        QObject::connect(saveMessageJob, &Akonadi::CollectionFetchJob::result, this, &ComposerViewCore::slotSaveMessage);
    } else {
        ...
        connect(create, &Akonadi::ItemCreateJob::result, this, &ComposerViewCore::slotCreateItemResult);
        ++m_pendingQueueJobs;
    }
}

void ComposerViewCore::slotSaveMessage(KJob *job)
{
    Akonadi::Collection target;
...
Akonadi::Collection ComposerViewCore::defaultSpecialTarget() const
{
    Akonadi::Collection target;
    switch (mSaveIn) {
    case MessageComposer::MessageSender::SaveInNone:
...
void ComposerViewCore::slotCreateItemResult(KJob *job)
{
    --m_pendingQueueJobs;
...
void ComposerViewCore::addAttachment(const QUrl &url, const QString &comment, bool sync)
{
    Q_UNUSED(comment)
...
void ComposerViewCore::addAttachment(const QString &name, const QString &filename, const QString &charset, const QByteArray &data, const QByteArray &mimeType)
{
...
void ComposerViewCore::addAttachmentPart(KMime::Content *partToAttach)
{
...
MessageComposer::Composer *ComposerViewCore::createSimpleComposer()
{
    auto composer = new MessageComposer::Composer;
    fillGlobalPart(composer->globalPart());
    m_editor->fillComposerTextPart(composer->textPart());
...
//-----------------------------------------------------------------------------
QString ComposerViewCore::to() const
{
    if (m_recipientsEditor) {
...
//-----------------------------------------------------------------------------
QString ComposerViewCore::cc() const
{
    if (m_recipientsEditor) {
...
//-----------------------------------------------------------------------------
QString ComposerViewCore::bcc() const
{
    if (m_recipientsEditor) {
...
QString ComposerViewCore::from() const
{
    return MessageComposer::Util::cleanedUpHeaderString(m_from);
}

QString ComposerViewCore::replyTo() const
{
    if (m_recipientsEditor) {
...
QString ComposerViewCore::subject() const
{
    return MessageComposer::Util::cleanedUpHeaderString(m_subject);
}

void ComposerViewCore::setParentWidgetForGui(QWidget *w)
{
    m_parentWidget = w;
}

void ComposerViewCore::setAttachmentController(MessageComposer::AttachmentControllerBase *controller)
{
    m_attachmentController = controller;
}

MessageComposer::AttachmentControllerBase *ComposerViewCore::attachmentController()
{
    return m_attachmentController;
}

void ComposerViewCore::setAttachmentModel(MessageComposer::AttachmentModel *model)
{
    m_attachmentModel = model;
}

MessageComposer::AttachmentModel *ComposerViewCore::attachmentModel()
{
    return m_attachmentModel;
}

void ComposerViewCore::setRecipientsEditor(MessageComposer::RecipientsEditor *recEditor)
{
    m_recipientsEditor = recEditor;
}

MessageComposer::RecipientsEditor *ComposerViewCore::recipientsEditor()
{
    return m_recipientsEditor;
}

void ComposerViewCore::setSignatureController(MessageComposer::SignatureController *sigController)
{
    m_signatureController = sigController;
}

MessageComposer::SignatureController *ComposerViewCore::signatureController()
{
    return m_signatureController;
}

void ComposerViewCore::setIdentityCombo(KIdentityManagement::IdentityCombo *identCombo)
{
    m_identityCombo = identCombo;
}

KIdentityManagement::IdentityCombo *ComposerViewCore::identityCombo()
{
    return m_identityCombo;
}

void ComposerViewCore::updateRecipients(const KIdentityManagement::Identity &ident,
...
void ComposerViewCore::identityChanged(const KIdentityManagement::Identity &ident, const KIdentityManagement::Identity &oldIdent, bool msgCleared)
{
...
void ComposerViewCore::setEditor(MessageComposer::RichTextComposerNg *editor)
{
    m_editor = editor;
    m_editor->document()->setModified(false);
}

MessageComposer::RichTextComposerNg *ComposerViewCore::editor() const
{
    return m_editor;
}

void ComposerViewCore::setTransportCombo(MailTransport::TransportComboBox *transpCombo)
{
    m_transport = transpCombo;
}

MailTransport::TransportComboBox *ComposerViewCore::transportComboBox() const
{
    return m_transport;
}

void ComposerViewCore::setIdentityManager(KIdentityManagement::IdentityManager *identMan)
{
    m_identMan = identMan;
}

KIdentityManagement::IdentityManager *ComposerViewCore::identityManager()
{
    return m_identMan;
}

void ComposerViewCore::setFcc(const Akonadi::Collection &fccCollection)
{
    if (m_fccCombo) {
    ...
    connect(checkFccCollectionJob, &KJob::result, this, &ComposerViewCore::slotFccCollectionCheckResult);
}

void ComposerViewCore::slotFccCollectionCheckResult(KJob *job)
{
    if (job->error()) {
        qCWarning(MESSAGECOMPOSER_LOG) << " void ComposerViewCore::slotFccCollectionCheckResult(KJob *job) error " << job->errorString();
...
void ComposerViewCore::setFccCombo(Akonadi::CollectionComboBox *fcc)
{
    m_fccCombo = fcc;
}

Akonadi::CollectionComboBox *ComposerViewCore::fccCombo() const
{
    return m_fccCombo;
}

void ComposerViewCore::setFrom(const QString &from)
{
    m_from = from;
}

void ComposerViewCore::setSubject(const QString &subject)
{
    m_subject = subject;
    if (mSendLaterInfo) {
...
void ComposerViewCore::setAutoSaveInterval(int interval)
{
    m_autoSaveInterval = interval;
}

void ComposerViewCore::setCryptoOptions(bool sign, bool encrypt, Kleo::CryptoMessageFormat format, bool neverEncryptDrafts)
{
    m_sign = sign;
    m_encrypt = encrypt;
...
void ComposerViewCore::setCharsets(const QVector<QByteArray> &charsets)
{
    m_charsets = charsets;
}

void ComposerViewCore::setMDNRequested(bool mdnRequested)
{
    m_mdnRequested = mdnRequested;
}

void ComposerViewCore::setUrgent(bool urgent)
{
    m_urgent = urgent;
}

QStringList ComposerViewCore::cleanEmailList(const QStringList &emails)
{
    QStringList clean;
...
int ComposerViewCore::autoSaveInterval() const
{
    return m_autoSaveInterval;
}

//-----------------------------------------------------------------------------
void ComposerViewCore::collectImages(KMime::Content *root)
{
...
//-----------------------------------------------------------------------------
bool ComposerViewCore::inlineSigningEncryptionSelected()
{
    if (!m_sign && !m_encrypt) {
        return false;
    }
...
bool ComposerViewCore::hasMissingAttachments(const QStringList &attachmentKeywords)
{
...
ComposerViewCore::MissingAttachment ComposerViewCore::checkForMissingAttachments(const QStringList &attachmentKeywords)
{
...
void ComposerViewCore::markAllAttachmentsForSigning(bool sign)
{
    if (m_attachmentModel) {
        const auto attachments = m_attachmentModel->attachments();
...
void ComposerViewCore::markAllAttachmentsForEncryption(bool encrypt)
{
    if (m_attachmentModel) {
        const auto attachments = m_attachmentModel->attachments();
        for (MessageCore::AttachmentPart::Ptr attachment : attachments) {
...
bool ComposerViewCore::determineWhetherToSign(bool doSignCompletely, Kleo::KeyResolver *keyResolver, bool signSomething, bool &result, bool &canceled)
{
    bool sign = false;
...
bool ComposerViewCore::determineWhetherToEncrypt(bool doEncryptCompletely,
...
void ComposerViewCore::setSendLaterInfo(SendLaterInfo *info)
{
    mSendLaterInfo.reset(info);
}

SendLaterInfo *ComposerViewCore::sendLaterInfo() const
{
    return mSendLaterInfo.get();
}

void ComposerViewCore::addFollowupReminder(const QString &messageId)
{
...
void ComposerViewCore::addSendLaterItem(const Akonadi::Item &item)
{
...
bool ComposerViewCore::requestDeleveryConfirmation() const
{
    return m_requestDeleveryConfirmation;
}

void ComposerViewCore::setRequestDeleveryConfirmation(bool requestDeleveryConfirmation)
{
    m_requestDeleveryConfirmation = requestDeleveryConfirmation;
}

KMime::Message::Ptr ComposerViewCore::msg() const
{
    return m_msg;
}
