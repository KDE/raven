...
QList<Thread*> Thread::fetchByFolder(QSqlDatabase &db, const QString &folderId, const QString &accountId, QObject *parent, int limit)
{
    QList<Thread*> threads;
    QSqlQuery query{db};

    query.prepare(QStringLiteral("SELECT ") + THREAD_TABLE + QStringLiteral(".* FROM ") + THREAD_TABLE +
        QStringLiteral(" INNER JOIN ") + THREAD_FOLDER_TABLE +
        QStringLiteral(" ON ") + THREAD_FOLDER_TABLE + QStringLiteral(".threadId = ") + THREAD_TABLE + QStringLiteral(".id") +
        QStringLiteral(" WHERE ") + THREAD_FOLDER_TABLE + QStringLiteral(".accountId = :accountId") +
        QStringLiteral(" AND ") + THREAD_FOLDER_TABLE + QStringLiteral(".folderId = :folderId") +
        QStringLiteral(" ORDER BY ") + THREAD_TABLE + QStringLiteral(".lastMessageTimestamp DESC") +
        QStringLiteral(" LIMIT :limit"));
    query.bindValue(QStringLiteral(":accountId"), accountId);
    query.bindValue(QStringLiteral(":folderId"), folderId);
    query.bindValue(QStringLiteral(":limit"), limit);

    if (!Utils::execWithLog(query, "fetching threads by folder")) {
        return threads;
    }

    while (query.next()) {
        threads.push_back(new Thread(parent, query));
    }
    return threads;
}

Thread* Thread::fetchById(QSqlDatabase &db, const QString &id, QObject *parent)
{
    QSqlQuery query{db};
    query.prepare(QStringLiteral("SELECT * FROM ") + THREAD_TABLE + QStringLiteral(" WHERE id = :id"));
    query.bindValue(QStringLiteral(":id"), id);

    if (!Utils::execWithLog(query, "fetching thread by id")) {
        return nullptr;
    }

    if (query.next()) {
        return new Thread(parent, query);
    }
    return nullptr;
}
