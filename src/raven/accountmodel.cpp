...
void AccountModel::removeAccount(int index)
{
    ...
    // Call D-Bus method to delete account data in the daemon
    OrgKdeRavenDaemonInterface iface(DBUS_SERVICE, DBUS_PATH, QDBusConnection::sessionBus());
    if (!iface.isValid()) {
        qWarning() << "D-Bus interface not available for account deletion";
        return;
    }

    QDBusPendingReply<bool> reply = iface.DeleteAccount(accountId);
    reply.waitForFinished();
    if (!reply.isValid() || !reply.value()) {
        qWarning() << "D-Bus DeleteAccount call failed:" << reply.error().message();
        return;
    }

    qDebug() << "Account deleted via D-Bus:" << accountId;
