...
impl ImapWorker {
    ...
    fn refresh_oauth2_token_if_needed(&mut self) -> Result<()> {
        ...
        // Store in KWallet
        if let Ok(kwallet) = KWalletClient::new() {
            let key = kwallet::oauth2_access_token_key(&self.account.id);
            if let Err(e) = kwallet.write_password(&key, &refreshed.access_token) {
                warn!("[{}] Failed to save token to KWallet: {}", self.account.email, e);
            }
        }
