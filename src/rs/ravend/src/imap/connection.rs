...
//! - `connect_with_kwallet`: High-level function that retrieves credentials from KWallet

use crate::kwallet::{KWalletClient, imap_password_key, oauth2_access_token_key, oauth2_refresh_token_key};
...
/// Connect to an IMAP server, retrieving credentials from KWallet
///
/// This is a higher-level function that:
/// 1. Retrieves credentials from KWallet based on authentication type
...
/// * `Ok(ImapSession)` - Authenticated IMAP session
/// * `Err(String)` - Error message if connection or authentication failed
pub fn connect_with_kwallet(account: &Account) -> Result<ImapSession, String> {
    // Create KWallet client
    let kwallet = KWalletClient::new()
        .map_err(|e| format!("Failed to open KWallet: {}", e))?;
    ...
    let (access_token, password) = match account.imap_authentication_type {
        AuthenticationType::OAuth2 => {
            ...
            let refresh_token = kwallet.read_password(&oauth2_refresh_token_key(&account.id))
            ...
            // Store new access token in KWallet
            let _ = kwallet.write_password(
        ...
        AuthenticationType::Plain => {
            let pass = kwallet.read_password(&imap_password_key(&account.id))
