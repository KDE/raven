...
pub fn upsert_message(conn: &Connection, message: &Message) -> Result<()> {
    ...
    conn.execute(
        "INSERT INTO message (id, accountId, data, folderId, threadId, headerMessageId,
                             gmailMessageId, gmailThreadId, subject, draft, unread, starred,
                             date, remoteUID)
...
pub fn get_message_by_uid(
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id, accountId, data, folderId, threadId, headerMessageId, gmailMessageId,
                    gmailThreadId, subject, draft, unread, starred, date, remoteUID
    ...
    let message = stmt
        .query_row(params![account_id, folder_id, remote_uid], |row| {
            ...
            Ok(Message {
                ...
                gmail_message_id: row.get(6)?,
                gmail_thread_id: row.get(7)?,
                subject: row.get(8)?,
                draft: row.get(9)?,
                unread: row.get(10)?,
                starred: row.get(11)?,
                date,
                remote_uid: row.get(13)?,
...
pub fn upsert_thread(conn: &Connection, thread: &Thread) -> Result<()> {
    conn.execute(
        "INSERT INTO thread (id, accountId, data, gmailThreadId, subject, snippet,
...
pub fn get_thread_by_id(conn: &Connection, thread_id: &str) -> Result<Option<Thread>> {
    let mut stmt = conn
        .prepare(
            "SELECT id, accountId, data, gmailThreadId, subject, snippet, unread, starred,
    ...
    let thread = stmt
        .query_row([thread_id], |row| {
            let first_ts_str: String = row.get(8)?;
            let last_ts_str: String = row.get(9)?;
...
pub fn delete_account_data(conn: &mut Connection, account_id: &str) -> Result<()> {
    ...
    tx.execute("DELETE FROM label WHERE accountId = ?1", [account_id])
        .with_context(|| format!("Failed to delete labels for account '{}'", account_id))?;
    ...
    tx.execute("DELETE FROM job WHERE accountId = ?1", [account_id])
        .with_context(|| format!("Failed to delete jobs for account '{}'", account_id))?;
...
pub fn upsert_file(conn: &Connection, file: &Attachment) -> Result<()> {
    conn.execute(
        "INSERT INTO file (id, accountId, messageId, fileName, partId, contentId, contentType, size, isInline, downloaded, data)
        ...
        params![
            ...
            file.is_inline,
            file.downloaded,
            Option::<String>::None, // data field is for future use
        ],
    )
